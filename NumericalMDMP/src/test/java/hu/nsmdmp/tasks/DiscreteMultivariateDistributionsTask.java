...
import static hu.nsmdmp.tasks.TaskUtils.createNormChebyshevTVector;
...
public class DiscreteMultivariateDistributionsTask {
	...
	public void discreteMultivariateDistributionsWithChebT() throws Exception {
		...
		Vector<Apfloat> nChebyTV = createNormChebyshevTVector(maxOrder, powerV, vectorSet);
		...
		double min = getMinCumProbMatrixElement(chebT, nChebyTV, f);
		double max = getMaxCumProbMatrixElement(chebT, nChebyTV, f);
